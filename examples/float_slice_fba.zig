//! Example: floatSlice (f64) flag with FixedBufferAllocator.
...
pub fn main() !void {
    ...
    var fs = pflag.FlagSet.init(alloc, "float-slice-fba");
    defer fs.deinit();
    ...
    std.debug.print("[float_slice/fba] OK: {d} items: [", .{scores.items.len});
